...
def signup(
    ...
    # 2) create them as a manager (with a placeholder share)
    session.add(Manager(ssn=new_emp.ssn, ownership_percentage=0.0))
    session.commit()

    # 3) rebalance ownership equally across all managers
    managers = session.exec(select(Manager)).all()
    count = len(managers)
    if count:
        equal_share = 100.0 / count
        for m in managers:
            m.ownership_percentage = equal_share
            session.add(m)
        session.commit()
...
def create_employee(
    ...
    session.commit()
    session.refresh(emp)
    session.add(Barista(ssn=emp.ssn))
    session.commit()
...
def update_employee(
    ...
    data = emp_up.dict(exclude_unset=True)

    # 1) If they sent a new SSN, propagate it first:
    new_ssn = data.pop("ssn", None)
    if new_ssn and new_ssn != ssn:
        old_ssn = emp.ssn
        emp.ssn = new_ssn
        session.add(emp)
        session.flush()  # make the new PK visible

        # re-point any manager or barista row
        session.exec(
            update(Manager)
            .where(Manager.ssn == old_ssn)
            .values(ssn=new_ssn)
        )
        session.exec(
            update(Barista)
            .where(Barista.ssn == old_ssn)
            .values(ssn=new_ssn)
        )

    # 2) Now update all the other fields
    for field, value in data.items():
        if field == "password":
            emp.password_hash = get_password_hash(value)
        else:
